...
<div class="min-h-screen bg-[#0a0e1a] text-gray-100">
    <div class="max-w-4xl mx-auto p-8">
        ...
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-white mb-2">Create Insurance Plan</h1>
            <p class="text-gray-400">Add a new insurance option for your rental cars</p>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.insurances.store') }}" method="POST" class="bg-[#1a2332] border border-gray-800 rounded-xl p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full px-4 py-3 bg-black/40 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-orange-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
