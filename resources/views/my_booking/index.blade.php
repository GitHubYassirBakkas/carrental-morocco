            ...
            @forelse($upcoming as $booking)
                <div class="mb-card mb-card--upcoming">
                    ...
                    <div class="mb-card-info">
                        ...
                        {{-- Status display --}}
                        <div class="mb-card-status">
                            @if($booking['status'] === 'pending')
                                <span class="mb-status-label mb-status-label--pending">{{ __('messages.booking_status_label_pending') }}</span>
                                <span class="mb-status-desc">{{ __('messages.booking_status_desc_pending') }}</span>
                            @elseif($booking['status'] === 'confirmed')
                                <span class="mb-status-label mb-status-label--confirmed">{{ __('messages.booking_status_label_confirmed') }}</span>
                                <span class="mb-status-desc">{{ __('messages.booking_status_desc_confirmed') }}</span>
                            @elseif($booking['status'] === 'active')
                                <span class="mb-status-label mb-status-label--active">{{ __('messages.booking_status_label_active') }}</span>
                                <span class="mb-status-desc">{{ __('messages.booking_status_desc_active') }}</span>
                            @elseif($booking['status'] === 'completed')
                                <span class="mb-status-label mb-status-label--completed">{{ __('messages.booking_status_label_completed') }}</span>
                                <span class="mb-status-desc">{{ __('messages.booking_status_desc_completed') }}</span>
                            @elseif($booking['status'] === 'cancelled')
                                @if($booking['refund_status'] === 'refunded')
                                    <span class="mb-status-label mb-status-label--refunded">{{ __('messages.booking_status_label_refunded') }}</span>
                                    <span class="mb-status-desc">{{ __('messages.booking_status_desc_refunded') }}</span>
                                @else
                                    <span class="mb-status-label mb-status-label--cancelled">{{ __('messages.booking_status_label_cancelled') }}</span>
                                    <span class="mb-status-desc">{{ __('messages.booking_status_desc_cancelled') }}</span>
                                @endif
                            @else
                                <span class="mb-status-label mb-status-label--muted">{{ ui_status($booking['status']) }}</span>
                            @endif
                        </div>
                        ...
                        <div class="mb-card-ref">{{ __('messages.booking_reference') }}: #{{ $booking['reference'] }}</div>
                    </div>
                    ...
                    <div class="mb-card-right">
                        ...
                        @if($booking['can_cancel'])
                            <form action="{{ route('bookings.cancel', $booking['id']) }}" method="POST" onsubmit="return confirm(@js(__('messages.cancel_booking_confirm')));">
                                @csrf
...
.mb-card-img img { width: 100%; height: 100%; object-fit: contain; object-position: center; transition: transform 0.5s ease; }
...
.mb-status-label--pending   { background: rgba(251,191,36,0.12); border: 1px solid rgba(251,191,36,0.3); color: #fbbf24; }
.mb-status-label--confirmed { background: rgba(96,165,250,0.12); border: 1px solid rgba(96,165,250,0.25); color: #60a5fa; }
.mb-status-label--active    { background: rgba(96,165,250,0.12); border: 1px solid rgba(96,165,250,0.25); color: #60a5fa; }
.mb-status-label--completed { background: rgba(74,222,128,0.12); border: 1px solid rgba(74,222,128,0.2); color: #4ade80; }
.mb-status-label--cancelled { background: rgba(248,113,113,0.12); border: 1px solid rgba(248,113,113,0.2); color: #f87171; }
.mb-status-label--refunded  { background: rgba(74,222,128,0.12); border: 1px solid rgba(74,222,128,0.2); color: #4ade80; }
.mb-status-label--muted    { background: rgba(255,255,255,0.05); border: 1px solid var(--b2); color: var(--muted); }
.mb-status-desc {
    font-size: 0.75rem;
    color: var(--muted);
    line-height: 1.3;
...
.mb-cancel-btn svg { width: 13px; height: 13px; }
.mb-cancel-btn:hover { background: rgba(248,113,113,0.18); border-color: rgba(248,113,113,0.4); }
